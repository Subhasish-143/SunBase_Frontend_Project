...
function handleClose(event) {
    ...
    const updatedFields = allFields.filter((field)=>field.id !== parseInt(fieldId));
...
buttons.forEach((button)=>{
    button.addEventListener("click", (e)=>{
        let newObj = null;
        if (e.target.parentNode.classList.contains("input")) newObj = {
            id: allFields.length + 1,
            type: "input",
            label: "Input text",
            value: "text"
        };
        else if (e.target.parentNode.classList.contains("textarea")) newObj = {
            id: allFields.length + 1,
            type: "textarea",
            label: "Textarea",
            value: "message"
        };
        else if (e.target.parentNode.classList.contains("select")) newObj = {
            id: allFields.length + 1,
            type: "select",
            label: "Select Option",
            value: [
...
function createField(type, label, value, id) {
    ...
    div.innerHTML = `
        ...
        <div class="input_field">
            ...
            </select>` : type === "textarea" ? `<textarea id="${type}" placeholder="${value}" ></textarea>` : `<input type="${type}" id="${type}" placeholder="${value}">`}
        </div>
    `;
    ...
    if (inputField) inputField.addEventListener("input", ()=>{
        saveFieldValue(id, inputField.value);
    });
    if (textareaField) textareaField.addEventListener("input", ()=>{
        saveFieldValue(id, textareaField.value);
    });
    if (selectField) selectField.addEventListener("change", ()=>{
...
function saveFieldValue(id, value) {
    ...
    if (fieldIndex !== -1) allFields[fieldIndex].value = value;
}
...
function handleDrop(e) {
    ...
    if (draggedElement != this) {
        ...
        document.querySelectorAll(".single_content").forEach((content)=>{
            ...
            let label = type === "input" ? "Input text" : type === "textarea" ? "Textarea" : "Select Option";
            let value = content.querySelector(".input_field").children[0].value;
            let obj = {
                id,
                label,
                type,
                value: value === "" ? type === "input" ? "text" : type === "textarea" ? "message" : [
                    "Select an option",
                    "Option 1",
                    "Option 2",
                    "Option 3"
...
let renderTotalList = ()=>{
    allFields.map((field)=>createField(field.type, field.label, field.value, field.id));
};
renderTotalList();
// EventListener for all the delete icons
document.addEventListener("click", (e)=>{
    if (e.target.classList.contains("fa-trash")) handleClose(e);
});
